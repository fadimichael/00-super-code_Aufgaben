...
const trenner = () => {
  let array = [];
  let arrayNumber = Math.ceil(text.length / 100);

  for (i = 0; i < text.length; i += 100) {
    let arraySegment = text.slice(i, i + 100);
    let arrayCounter = text.indexOf();

    array.push(`${arraySegment} -${arrayCounter}of ${arrayNumber}`);
    // array.push(`${arraySegment} -${arrayCounter}of ${arrayNumber}`);
  }
  console.log(array);
};
...
for (
  ...
  array.length;
  target.push(array.splice(0, 100).join(""))
);
