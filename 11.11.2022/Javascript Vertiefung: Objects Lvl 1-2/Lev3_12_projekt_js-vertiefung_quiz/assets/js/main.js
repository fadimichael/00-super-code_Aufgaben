...
let createQuestion = () => {
  data.forEach((element) => {
    ...
    wrapperDiv.classList.add("question-wrapper");
    ...
    theQuestion.textContent = element.question;
    wrapperDiv.appendChild(theQuestion);

    let answerChoises = document.createElement(`div`);
    answerChoises.classList.add(`choises`);
    for (let i = 0; i < element.choice.length; i++) {
      let button = document.createElement(`button`);
      button.textContent = element.choice[i];
      answerChoises.appendChild(button);
      button.addEventListener("click", () => {
        if (element.answer.toString() == button.textContent) {
    ...
    wrapperDiv.appendChild(answerChoises);
  });
};
